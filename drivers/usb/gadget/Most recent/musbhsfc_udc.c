...
static void udc_enable(struct musbhsfc_udc *dev)
{
	unsigned long gpio_phys_addr;
	unsigned long gpio_end_addr;
	unsigned long gpio_base_len;
	int ep=0;
	...
	DEBUG("%s, %p\n", __FUNCTION__, dev);
	/*
	gpio_phys_addr = GPIO_PHYS_START;
	gpio_end_addr = GPIO_PHYS_END;
	gpio_base_len = gpio_end_addr - gpio_phys_addr + 1;
	
	dev->gpio_base = ioremap(gpio_phys_addr, gpio_base_len);
	if (!dev->gpio_base)
	{
	...
	printk(KERN_ERR "Setting up GPIO33\n");
	
	amcc440ep_clear_reg32(&dev->gpio_base->osrl, SET_BIT32(2));
	amcc440ep_clear_reg32(&dev->gpio_base->osrl, SET_BIT32(3));
	amcc440ep_clear_reg32(&dev->gpio_base->tsrl, SET_BIT32(2));
	amcc440ep_clear_reg32(&dev->gpio_base->tsrl, SET_BIT32(3));
	amcc440ep_clear_reg32(&dev->gpio_base->odr, SET_BIT32(1));
	amcc440ep_set_reg32(&dev->gpio_base->tcr, SET_BIT32(1));
	amcc440ep_set_reg32(&dev->gpio_base->orr, SET_BIT32(1));
	*/
	dev->usb2_device = 0;
	dev->speed_set = 0;
			  ...
			  struct usb_gadget_driver *driver)
{
	...
	if (driver) {
		...
		printk("driver disconnect -stop activity\n");
...
static irqreturn_t musbhsfc_udc_irq(int irq, void *_dev)
{
	...
	for (;;) {
		...
		{
		    printk("driver disconnect -interrupt\n");
