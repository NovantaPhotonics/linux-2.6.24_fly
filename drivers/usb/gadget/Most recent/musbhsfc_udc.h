...
// sbs Changed to accomodate 4 data points ctl + 4
...
#define GPIO_PHYS_START		0x0EF600C00
#define GPIO_PHYS_END		0x0EF600C44	
...
struct musbhsfc_udc {
	...
	struct amcc440ep_gpio_reg __iomem  *gpio_base;
	unsigned char usb_address;
	unsigned usb2_device:1;
	unsigned speed_set:1;
};

static inline struct musbhsfc_udc *to_udc(struct usb_gadget *g)
{
    return container_of(g, struct musbhsfc_udc, gadget);
}
