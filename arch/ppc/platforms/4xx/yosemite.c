...
{
        printk("Setup Yosemite SPI Flash info\n");
...
static struct spi_board_info yosemite_spi_bi[] = {
...
static int __init yosemite_spi_init(void)
{
        printk("Registering Yosemite SPI Board info\n");
	int numentries = sizeof(yosemite_spi_bi)/sizeof(struct spi_board_info);
	int res = 0;

	if (numentries)
		res = spi_register_board_info(yosemite_spi_bi, numentries);
	return res;
}
device_initcall(yosemite_spi_init);
#endif
